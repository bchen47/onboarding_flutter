...
import 'package:prueba/blocs/training_list_class/training_list_class_bloc.dart';
...
import 'package:prueba/screens/training_individual_class/training_individual_class_page.dart';
...
class TrainingClassListPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
        create: (BuildContext context) =>
            TrainingClassBloc(classRepository: TrainingClassRepository()),
        child: Scaffold(
            body: Padding(
          padding: const EdgeInsets.all(12),
          child: BlocBuilder<AuthenticationBloc, AuthenticationState>(
              builder: (context, state) {
            context.read<TrainingListClassBloc>().add(GetTrainingListClasses(
  ...
  Widget home(context) {
    return BlocBuilder<TrainingListClassBloc, TrainingListClassState>(
        builder: (context, state) {
      return ListView.separated(
        padding: const EdgeInsets.all(8),
        shrinkWrap: true,
        itemCount: state.trainingListClass.attributes.length,
        itemBuilder: (BuildContext context, int index) {
          return Container(
              child: state.trainingListClass.attributes.isNotEmpty
                  ? card(context, state.trainingListClass.attributes[index],
                      state.trainingListClass.trainers)
                  : Container());
        },
                  ...
                  MaterialPageRoute(
                      builder: (context) => BlocProvider(
                            create: (_) => TrainingClassBloc(
                                classRepository: TrainingClassRepository()),
